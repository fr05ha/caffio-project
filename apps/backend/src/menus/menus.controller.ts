...
export class MenusController {
  ...
  @Post('items')
  @ApiBody({ schema: { properties: { menuId: { type: 'number' }, name: { type: 'string' }, description: { type: 'string' }, price: { type: 'number' }, currency: { type: 'string' } } } })
  async addMenuItem(@Body() data: { menuId: number; name: string; description?: string; price: number; currency?: string }) {
    return this.menusService.addMenuItem(data);
  }

  @Put('items/:id')
  @ApiParam({ name: 'id', type: Number })
  @ApiBody({ schema: { properties: { name: { type: 'string' }, description: { type: 'string' }, price: { type: 'number' }, currency: { type: 'string' } } } })
  async updateMenuItem(@Param('id', ParseIntPipe) id: number, @Body() data: { name?: string; description?: string; price?: number; currency?: string }) {
